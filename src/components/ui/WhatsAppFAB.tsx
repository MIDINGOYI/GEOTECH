      ...
      <AnimatePresence>
        {visible && !dismissed && (
          <motion.div
            ...
            className="bg-white shadow-xl rounded-sm px-4 py-3 max-w-[200px] relative"
          >
            <button
              onClick={() => setDismissed(true)}
              className="absolute -top-2 -right-2 bg-brand-gray text-white rounded-full w-5 h-5 flex items-center justify-center"
              aria-label="Fermer"
            >
              <X size={10} />
            </button>
            <p className="font-body text-xs text-brand-navy font-semibold leading-snug">
